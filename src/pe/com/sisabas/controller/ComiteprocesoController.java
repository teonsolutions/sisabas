...
@Component(value ="comiteproceso")
@Scope(value = "session")
public class ComiteprocesoController extends BaseController {

	private static final long serialVersionUID = 1L;
	private StreamedContent file;
	private Comiteproceso comiteprocesoB;
	private Comiteproceso comiteproceso;
	...
	@Autowired
	public ComiteprocesoBusiness business;



	//Nota: idOpcion, sicuopcion estan en BaseController
	private String idOpcionText = "OPC_COMITEPROCESO";
	public ComiteprocesoController(){
			comiteproceso = new Comiteproceso();
	...
	public void buscar() {
		try {
			List<String> ordenListaCampos = Arrays.asList("A1.idcomiteproceso");
			comiteprocesoB.setOrdenListaCampos(ordenListaCampos);
			comiteprocesoB.setOrdenTipo("DESC");
			...
			listaComiteproceso = business.selectDynamicFull(comiteprocesoB);
			setEsSeleccionado(false);
			setSelectedComiteproceso(null);
	...
	public void irRegistrar() {
		...
		try {
			...
			resetRegisterForm();
			accion = REGISTRAR;
			titulo = "Comite Proceso » " + REGISTRAR;
			comiteproceso = new Comiteproceso();


			comiteproceso.setIdcomiteproceso(new java.lang.Integer(0));
			comiteproceso.setIdcomiteproceso((int)utilsBusiness.getNextSeqTemporal(pe.com.sisabas.resources.Sequence.SEQ_COMITEPROCESO).longValue());
	...
	public void irEditar() {
		...
		try {
			...
			resetRegisterForm();
			validateSelectedRow();
			updateCharToBoolean(comiteproceso);
			comiteproceso.roundBigDecimals();
			accion = EDITAR;
			titulo = "Comite Proceso » " + EDITAR;
	...
	public void irImprimir() {
		...
		try {
			...
			//resetRegisterForm();
			accion = IMPRIMIR;
			titulo = "Comite Proceso » " + IMPRIMIR;
	...
	public void irAnular() {
		...
		try {
			...
			resetAnulacionForm();
			validateSelectedRow();
			if(comiteproceso.getEstadoauditoria().equals("1")){
				accion = ANULAR;
			}else{
				accion = ACTIVAR;
			}
			titulo = "Comite Proceso » " + accion;
	...
	public void anular() {
		try {
			Comiteproceso record= new Comiteproceso();
			if(comiteproceso.getEstadoauditoria().equals("1")){
				record.setEstadoauditoria("0");
			}else{
				record.setEstadoauditoria("1");
			}
			record.setIdcomiteproceso(comiteproceso.getIdcomiteproceso());
			...
			REGISTER_ERROR();
			addMessageKey("msgsComiteprocesoA", e.getMessage(),
			FacesMessage.SEVERITY_ERROR);
			...
			REGISTER_ERROR();
			addErrorMessageKey("msgsComiteprocesoA", e);
		}
	}
	...
	public void aceptar() {
		...
		try {
			if (accion.equals(REGISTRAR)){
				comiteproceso.setUsuariocreacionauditoria(getUserLogin());
				comiteproceso.setEquipoauditoria(getRemoteAddr());
				comiteproceso.setProgramaauditoria(pe.com.sisabas.resources.Utils.obtenerPrograma(this.getClass()));
				business.insertBasic(comiteproceso);
			}else{
				comiteproceso.setUsuariomodificacionauditoria(getUserLogin());
				comiteproceso.setEquipoauditoria(getRemoteAddr());
				comiteproceso.setProgramaauditoria(pe.com.sisabas.resources.Utils.obtenerPrograma(this.getClass()));
				business.updateByPrimaryKeyBasic(comiteproceso);
			}
			...
			REGISTER_ERROR();
			addMessageKey("msgsComiteprocesoR", e.getMessage(),
			FacesMessage.SEVERITY_ERROR);
		} catch (BusinessException e) {
			REGISTER_ERROR();
			addMessageKey("msgsComiteprocesoR", e.getMessage(),
			...
			REGISTER_ERROR();
			addErrorMessageKey("msgsComiteprocesoR", e);
		}
	}
	...
	public void eliminar() {
		try {
			validateSelectedRow();
			comiteproceso.setUsuariomodificacionauditoria(getUserLogin());
			business.deleteByPrimaryKeyBasic(comiteproceso);
			showGrowlMessageSuccessfullyCompletedAction();
			buscar();
		} catch (ValidateException e) {
			addMessageKey("msgsForm", e.getMessage(),
	...
	public void validateSelectedRow() throws UnselectedRowException, CloneNotSupportedException {
		if (selectedComiteproceso == null)
			throw new UnselectedRowException(Messages.getString("no.record.selected"));
		else
			comiteproceso = (Comiteproceso)selectedComiteproceso.clone();
	}
	...
	public String loadPage(){
		buscar();
		return "/pages/comiteproceso/comiteprocesoBuscar.xhtml?faces-redirect=true";
	}

	public void resetRegisterForm(){
		reset("frmComiteprocesoRegistrar:panelC");
	}
	public void resetAnulacionForm(){
		reset("frmComiteprocesoAnular:panelDetailC");
	}


	public void updateCharToBoolean(Comiteproceso record) throws Exception {
	}

	public void setComiteproceso(Comiteproceso comiteproceso){
		this.comiteproceso=comiteproceso;
	}

	public Comiteproceso getComiteproceso(){
		return comiteproceso;
	}
