...
@Component(value ="vcertificacion")
@Scope(value = "session")
public class VcertificacionController extends BaseController {

	private static final long serialVersionUID = 1L;
	private StreamedContent file;
	private Vcertificacion vcertificacionB;
	private Vcertificacion vcertificacion;
	...
	@Autowired
	public VcertificacionBusiness business;



	//Nota: idOpcion, sicuopcion estan en BaseController
	private String idOpcionText = "OPC_VCERTIFICACION";
	public VcertificacionController(){
			vcertificacion = new Vcertificacion();
	...
	public void buscar() {
		try {
			List<String> ordenListaCampos = Arrays.asList("A1.nrocertifica");
			vcertificacionB.setOrdenListaCampos(ordenListaCampos);
			vcertificacionB.setOrdenTipo("DESC");
			...
			listaVcertificacion = business.selectDynamicFull(vcertificacionB);
			setEsSeleccionado(false);
			setSelectedVcertificacion(null);
	...
	public void irRegistrar() {
		...
		try {
			...
			resetRegisterForm();
			accion = REGISTRAR;
			titulo = "VCertificacion » " + REGISTRAR;
			vcertificacion = new Vcertificacion();


			vcertificacion.setVcertificacion(new java.lang.Integer(0));
			vcertificacion.setVcertificacion((int)utilsBusiness.getNextSeqTemporal(pe.com.sisabas.resources.Sequence.SEQ_VCERTIFICACION).longValue());
	...
	public void irEditar() {
		...
		try {
			...
			resetRegisterForm();
			validateSelectedRow();
			updateCharToBoolean(vcertificacion);
			vcertificacion.roundBigDecimals();
			accion = EDITAR;
			titulo = "VCertificacion » " + EDITAR;
	...
	public void irImprimir() {
		...
		try {
			...
			//resetRegisterForm();
			accion = IMPRIMIR;
			titulo = "VCertificacion » " + IMPRIMIR;
	...
	public void irAnular() {
		...
		try {
			...
			resetAnulacionForm();
			validateSelectedRow();
			if(vcertificacion.getEstadoauditoria().equals("1")){
				accion = ANULAR;
			}else{
				accion = ACTIVAR;
			}
			titulo = "VCertificacion » " + accion;
	...
	public void anular() {
		try {
			Vcertificacion record= new Vcertificacion();
			if(vcertificacion.getEstadoauditoria().equals("1")){
				record.setEstadoauditoria("0");
			}else{
				record.setEstadoauditoria("1");
			}
			record.setVcertificacion(vcertificacion.getVcertificacion());
			...
			REGISTER_ERROR();
			addMessageKey("msgsVcertificacionA", e.getMessage(),
			FacesMessage.SEVERITY_ERROR);
			...
			REGISTER_ERROR();
			addErrorMessageKey("msgsVcertificacionA", e);
		}
	}
	...
	public void aceptar() {
		...
		try {
			if (accion.equals(REGISTRAR)){
				vcertificacion.setUsuariocreacionauditoria(getUserLogin());
				vcertificacion.setEquipoauditoria(getRemoteAddr());
				vcertificacion.setProgramaauditoria(pe.com.sisabas.resources.Utils.obtenerPrograma(this.getClass()));
				business.insertBasic(vcertificacion);
			}else{
				vcertificacion.setUsuariomodificacionauditoria(getUserLogin());
				vcertificacion.setEquipoauditoria(getRemoteAddr());
				vcertificacion.setProgramaauditoria(pe.com.sisabas.resources.Utils.obtenerPrograma(this.getClass()));
				business.updateByPrimaryKeyBasic(vcertificacion);
			}
			...
			REGISTER_ERROR();
			addMessageKey("msgsVcertificacionR", e.getMessage(),
			FacesMessage.SEVERITY_ERROR);
		} catch (BusinessException e) {
			REGISTER_ERROR();
			addMessageKey("msgsVcertificacionR", e.getMessage(),
			...
			REGISTER_ERROR();
			addErrorMessageKey("msgsVcertificacionR", e);
		}
	}
	...
	public void eliminar() {
		try {
			validateSelectedRow();
			vcertificacion.setUsuariomodificacionauditoria(getUserLogin());
			business.deleteByPrimaryKeyBasic(vcertificacion);
			showGrowlMessageSuccessfullyCompletedAction();
			buscar();
		} catch (ValidateException e) {
			addMessageKey("msgsForm", e.getMessage(),
	...
	public void validateSelectedRow() throws UnselectedRowException, CloneNotSupportedException {
		if (selectedVcertificacion == null)
			throw new UnselectedRowException(Messages.getString("no.record.selected"));
		else
			vcertificacion = (Vcertificacion)selectedVcertificacion.clone();
	}
	...
	public String loadPage(){
		buscar();
		return "/pages/vcertificacion/vcertificacionBuscar.xhtml?faces-redirect=true";
	}

	public void resetRegisterForm(){
		reset("frmVcertificacionRegistrar:panelC");
	}
	public void resetAnulacionForm(){
		reset("frmVcertificacionAnular:panelDetailC");
	}


	public void updateCharToBoolean(Vcertificacion record) throws Exception {
	}

	public void setVcertificacion(Vcertificacion vcertificacion){
		this.vcertificacion=vcertificacion;
	}

	public Vcertificacion getVcertificacion(){
