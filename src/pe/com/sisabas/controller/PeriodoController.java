...
public class PeriodoController extends BaseController {
	...
	@Autowired
	public PeriodoBusiness business;



	//Nota: idOpcion, sicuopcion estan en BaseController
	private String idOpcionText = "OPC_PERIODO";
	...
	public void buscar() {
		try {
			...
			pe.com.sisabas.resources.Utils.convertPropertiesStringToUppercase(periodoB); //pasa a mayusculas los datos para la busqueda
			listaPeriodo = business.selectDynamicFull(periodoB);
	...
	public void irRegistrar() {
		...
		try {
			...
			resetRegisterForm();
			accion = REGISTRAR;
			titulo = "Periodo » " + REGISTRAR;
			periodo = new Periodo();


			periodo.setIdperiodo(new java.lang.Integer(0));
			periodo.setIdperiodo((int)utilsBusiness.getNextSeqTemporal(pe.com.sisabas.resources.Sequence.SEQ_PERIODO).longValue());
	...
	public void irEditar() {
		...
		try {
			...
			resetRegisterForm();
			validateSelectedRow();
			updateCharToBoolean(periodo);
			periodo.roundBigDecimals();
			accion = EDITAR;
			titulo = "Periodo » " + EDITAR;
	...
	public void irAnular() {
		...
		try {
			...
			resetAnulacionForm();
			validateSelectedRow();
			if(periodo.getEstadoauditoria().equals("1")){
				accion = ANULAR;
			}else{
				accion = ACTIVAR;
			}
			titulo = "Periodo » " + accion;
	...
	public void anular() {
		try {
			Periodo record= new Periodo();
			if(periodo.getEstadoauditoria().equals("1")){
				record.setEstadoauditoria("0");
			}else{
				record.setEstadoauditoria("1");
			}
			record.setIdperiodo(periodo.getIdperiodo());
			...
			REGISTER_ERROR();
			addMessageKey("msgsPeriodoA", e.getMessage(),
			FacesMessage.SEVERITY_ERROR);
			...
			REGISTER_ERROR();
			addErrorMessageKey("msgsPeriodoA", e);
		}
	}
	...
	public void aceptar() {
		...
		try {
			if (accion.equals(REGISTRAR)){
				periodo.setUsuariocreacionauditoria(getUserLogin());
				periodo.setEquipoauditoria(getRemoteAddr());
				periodo.setProgramaauditoria(pe.com.sisabas.resources.Utils.obtenerPrograma(this.getClass()));
				business.insertBasic(periodo);
			}else{
				periodo.setUsuariomodificacionauditoria(getUserLogin());
				periodo.setEquipoauditoria(getRemoteAddr());
				periodo.setProgramaauditoria(pe.com.sisabas.resources.Utils.obtenerPrograma(this.getClass()));
				business.updateByPrimaryKeyBasic(periodo);
			}
			...
			REGISTER_ERROR();
			addMessageKey("msgsPeriodoR", e.getMessage(),
			FacesMessage.SEVERITY_ERROR);
		} catch (BusinessException e) {
			REGISTER_ERROR();
			addMessageKey("msgsPeriodoR", e.getMessage(),
			...
			REGISTER_ERROR();
			addErrorMessageKey("msgsPeriodoR", e);
		}
	}
	...
	public void eliminar() {
		try {
			validateSelectedRow();
			periodo.setUsuariomodificacionauditoria(getUserLogin());
			business.deleteByPrimaryKeyBasic(periodo);
			showGrowlMessageSuccessfullyCompletedAction();
			buscar();
		} catch (ValidateException e) {
			addMessageKey("msgsForm", e.getMessage(),
	...
	public void validateSelectedRow() throws UnselectedRowException, CloneNotSupportedException {
		if (selectedPeriodo == null)
			...
			periodo = (Periodo)selectedPeriodo.clone();
	}
	...
	public void resetRegisterForm(){
		reset("frmPeriodoRegistrar:panelC");
	}
	public void resetAnulacionForm(){
		reset("frmPeriodoAnular:panelDetailC");
	}


	public void updateCharToBoolean(Periodo record) throws Exception {
		if( periodo.getEstadoauditoria()!=null  && periodo.getEstadoauditoria().equalsIgnoreCase("1")){
			periodo.setBooleanestadoauditoria(true);
		}else {
			periodo.setBooleanestadoauditoria(false);
		}
